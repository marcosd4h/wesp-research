...
Per-rule records under `Rules\{rule-guid}` carry `version` (REG_DWORD, must equal `1`), `event_type` (REG_DWORD), `lifetime` (REG_DWORD; `0` transient, `1` persistent), `order_group` (REG_QWORD), `enabled` (REG_DWORD), `flags` (REG_DWORD), and `is_top_level` (REG_DWORD), plus a 32-byte `action` REG_BINARY and optional `filter` and `modify` REG_BINARY values. The `action` deserializer requires exactly 32 bytes with a leading QWORD selector that must not exceed 8; selector 3 additionally requires the final QWORD to be less than 5. Values outside these bounds return `STATUS_INVALID_PARAMETER`.
...
Registry state lives under the `Wesp` service key (`Services\Wesp`). The service root is created with full access and the `Policy` subtree opens read-only (`0x20019`); the `PersistedStore/Rules/boot_id/Clients` subtree is created with full access. Per-client GUID subkeys carry a version DWORD that must equal `1`, plus altitude and client-name strings; `notification_version` and `queue_typemax_capacity_bytes` DWORDs and the `boot_id` recovery oracle travel with the persisted state. Per-client rules, event queues, and collections persist through transacted writes with commit/rollback, and rule updates take the pushlock exclusively while readers hold it shared. Each driver loads its own copy of persisted clients at its own `DriverEntry`; the handoff is schema-shared and registry-persisted, not live-shared.
...
The durable store is a driver-owned `PersistedStore` registry subtree with fixed category subkeys (`Clients`, `Rules`, `Collections`, `EventQueues`) and per-object GUID subkeys holding `REG_BINARY` payloads. The driver initialization entry point opens the store root with `KEY_ALL_ACCESS`; the root is `HKLM\SYSTEM\CurrentControlSet\Services\Wesp\PersistedStore`. The service key DACL grants `NT AUTHORITY\SYSTEM` and `BUILTIN\Administrators` full control and `BUILTIN\Users` read access, and the `PersistedStore` subtree inherits that DACL. Administrators are not DACL-excluded, so the `CmRegisterCallbackEx` registry callback is the runtime mediation point.
...
| RecoveryMonitor                          | The `wesp_lib::recovery` routine that recursively wipes `HKLM\SYSTEM\CurrentControlSet\Services\Wesp\PersistedStore` when the `boot_id` oracle indicates stale state. ( Boot-Time Persisted-Store Recovery and WNF BootMonitor)                                                                                                                                                                                                                                                                                |
